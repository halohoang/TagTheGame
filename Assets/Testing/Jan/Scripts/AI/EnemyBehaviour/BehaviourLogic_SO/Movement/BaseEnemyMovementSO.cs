    ...
    public class BaseEnemyMovementSO : ScriptableObject    //Todo: continue here, implement Base Logic for EnemyMovementBehaviour, JM (04.11.24)
    {
        ...
        public virtual void ExecuteFrameUpdate()
        {
            ...
            if (_behaviourCtrl.IsSomethingAlarmingHappening || _behaviourCtrl.IsTargetDetected)
            {
                ...
                Debug.Log($"<color=orange>AI-Behav of '{_behaviourCtrl.gameObject.name}'</color>: State-Transition from '<color=orange>Movement</color>' to '<color=orange>Alert</color>' " +
                    $"should have been happend now!");
                return;
            }

            #region old transit to chase state
